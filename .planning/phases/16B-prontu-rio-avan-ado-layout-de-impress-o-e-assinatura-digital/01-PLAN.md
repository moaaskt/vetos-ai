...
<task type="auto">
  ...
  <action>
    ...
    2. Modelo 'Prescription' (Receita Médica) com:
       ...
       - documentHash (String, nullable, @unique)
       - signedAt (DateTime, nullable)
       - verificationUrl (String, nullable)
       - verificationQrCode (String, nullable)
...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - A assinatura do documento gera um hash SHA-256 de 64 caracteres, armazena a URL pública de verificação em 'verificationUrl' e uma string base64 válida na propriedade 'verificationQrCode' (QR Code).
...
<task type="auto">
  ...
  <action>
    1. Criar a estrutura do módulo de termos sob 'backend/src/consent-terms/':
       ...
       - DTO para criação de termos contendo os campos obrigatórios (petId, finalText) e opcionais (appointmentId, consentTemplateId).
       ...
       - No método de assinatura '/:id/sign': Calcular hash SHA-256 a partir do 'finalText' e metadados, armazenar a URL '/verify/{hash}' em 'verificationUrl', gerar o QR code base64 correspondente à URL de verificação para preencher 'verificationQrCode', alterar status para SIGNED e aplicar regras de imutabilidade similares às de receitas.
  ...
  <acceptance_criteria>
    ...
    - A assinatura do termo gera o hash SHA-256, a URL pública de verificação em 'verificationUrl' e o QR Code de verificação em 'verificationQrCode', tornando o documento imutável.
